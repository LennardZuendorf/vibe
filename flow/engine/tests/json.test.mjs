...
import { readdirSync, readFileSync, existsSync, writeFileSync, mkdirSync, linkSync, statSync } from 'node:fs';
...
// js-core/8 final review, M2 — the whole reason this helper exists ("a crash
// mid-write never truncates the cursor", tech.md) was pinned by NOTHING:
// replacing the temp-file+rename with a direct `writeFileSync(filePath, body)`
// left the entire suite green. The two tests below it were named for the
// contract without testing it — ":91 overwrites in place (rename semantics)"
// passes without a rename, and "leaves no stray temp file" passes trivially when
// no temp file is ever created.
//
// This discriminates without spying on `fs`, so it holds no matter how the
// helper is spelled: a HARD LINK captures the target's inode before the write.
// rename(2) replaces the NAME with a different inode, so the alias keeps the old
// bytes and the two inodes differ. A direct write mutates the shared inode in
// place, so the alias would see the new bytes — which is precisely the
// truncatable path this helper exists to avoid.
test('writeJsonAtomic: replaces the target by RENAME, never writing through the existing inode', () => {
  const sandbox = makeSandbox();
  try {
    const target = path.join(sandbox.dir, 'atomic.json');
    writeJsonAtomic(target, { v: 1 });

    const alias = path.join(sandbox.dir, 'atomic.alias.json');
    linkSync(target, alias);
    const inoBefore = statSync(target).ino;
    assertEqual(statSync(alias).ino, inoBefore, 'precondition: the hard link shares the target inode');

    writeJsonAtomic(target, { v: 2 });

    assertEqual(JSON.parse(readFileSync(target, 'utf8')), { v: 2 }, 'the target must carry the new content');
    assertEqual(
      JSON.parse(readFileSync(alias, 'utf8')),
      { v: 1 },
      'the pre-existing inode must be UNTOUCHED — writing through it is the truncatable path ' +
        'writeJsonAtomic exists to avoid (tech.md: "a crash mid-write never truncates the cursor")',
    );
    assert(
      statSync(target).ino !== inoBefore,
      'the target must be a NEW inode moved into place, not the old one written through',
    );
  } finally {
    sandbox.cleanup();
  }
