...
Written by the setup interview; read by the skill at session start to adjust
behavior. All fields are optional with defined defaults so the skill degrades
gracefully on absent or partial files.

```yaml
# .spec/.config.yaml — generated by /spec setup
# Edit manually or re-run /spec setup to update.
...
- `suggest-superpowers: true` — tips on by default

**How the skill reads it:** At session start (or on any `/spec` invocation),
the skill checks for `.spec/.config.yaml`. If present, it loads the config
and adjusts:
- `suggest-superpowers: false` → suppress all "Superpower tip" and "Superpower
  tip (Step N):" callouts; self-execute every step without offering
- `vibe-flow: true` → note that caveman level is auto-managed; don't ask
- `caveman: lite` → apply lite output profile by default; tell the user
- Specific `superpowers.x: false` → don't offer that executor; route to
  self-execution for that step silently

### Interview flow in SKILL.md `## Setup`

The `## Setup` section of SKILL.md SHALL be replaced with an explicit
interview flow the agent runs before calling `setup.sh`:
...
4. On confirm: write `.spec/.config.yaml`; then run `setup.sh` to create
   entrypoint templates. Report what was created.
...
The mechanical script stays unchanged — it only copies templates. The interview
is owned by the SKILL.md agent layer, not the bash script. This preserves the
ability to run `bash setup.sh` directly and get the non-interactive behavior.
