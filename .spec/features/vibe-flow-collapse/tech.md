...
└── skills/
    ...
    └── vibe/
        ...
        ├── state-machine.json        ← now in .agents/skills/vibe/
        ├── state.example.json        ← now in .agents/skills/vibe/
...
The old `flow/` dir is **removed**. After this feature `.agents/` contains only `skills/`.
...
Every script derives its paths from `SCRIPT_DIR` via `cd ..` chains. Moving the scripts to
`.agents/skills/vibe/scripts/` (from the old flow/scripts/) adds one directory level,
...
|---|---|---|---|---|
| `set-state.sh` | `FLOW_DIR`→MACHINE,STATE | `FLOW_DIR=SCRIPT_DIR/..` (old flow) | `SKILL_DIR=SCRIPT_DIR/..` (=`vibe/`) | rename only; MACHINE/STATE follow |
| `validate-state.sh` | `FLOW_DIR`→MACHINE,STATE; example hint | same | same | rename only |
| `detect-context.sh` | `FLOW_DIR`→MACHINE,STATE; **cursor block pattern** | same + literal old cursor path (line ~79) | `SKILL_DIR`; pattern → `.agents/skills/vibe/state.json` | rename **+ block path (R9)** |
...
Also update the in-script comment/usage/fallback strings that still reference the old path
...
|---|---|---|
| `hooks/user-prompt-submit-inject.sh` | old: `$ROOT/.agents/skills/vibe/scripts/orders.sh` | `$ROOT/.agents/skills/vibe/scripts/orders.sh` |
| `hooks/pre-tool-use-guard.sh` | old: `$ROOT/.agents/skills/vibe/scripts/detect-context.sh` | `$ROOT/.agents/skills/vibe/scripts/detect-context.sh` |
| `hooks/stop-gate.sh` | old: `$ROOT/.agents/skills/vibe/scripts/detect-context.sh` | `$ROOT/.agents/skills/vibe/scripts/detect-context.sh` |
| `hooks/hooks.json` | description: "Thin shells over vibe/scripts/" | "…over .agents/skills/vibe/scripts/" |
| `commands/flow.md` | reads old state.json, state-machine.json; runs old set-state.sh | `.agents/skills/vibe/...` |
...
| Lines | Change |
|---|---|
| 8, 10, 16–19 (comments) | drop the separate old-flow mention; cursor path → `.agents/skills/vibe/state.json` |
| 51–68 (copy + snapshot/restore) | remove old `cp -R` for the flow dir (line 61); snapshot/restore target cursor at `.agents/skills/vibe/state.json`. `cp -R .agents/skills` now also carries the source's gitignored `state.json` if present — the existing restore-or-`rm` branch already neutralises that (restore target's, else `rm` the inherited one) |
| 77–79 (chmod) | drop old flow scripts chmod; keep `skills/spec` + `skills/vibe` script chmod |
...
| 119 (final note) | "hooks read .agents/skills/vibe" — already updated |
...
```diff
-.agents/skills/vibe/state.json   # old gitignore entry (now updated)
...
| File | Refs | Change |
|---|---|---|
| old `flow/scripts/*.sh` (×6) | self-location + literals | moved to `.agents/skills/vibe/scripts/`; per-script fix applied |
| old `flow/state-machine.json`, `state.example.json` | — | moved to `.agents/skills/vibe/` |
...
| `.agents/skills/vibe/setup.md` | 8 | scaffold steps (`set-state.sh`, ensure-`.agents/skills/vibe/`-exists, seed cursor, gitignore, `regen`/`check-skills`) → new layout |
| `.agents/skills/vibe/{feature,compound,amend,verify,strategy,quick}.md` | 12 | `Read .agents/skills/vibe/state.json` + `set-state.sh`/`detect-context.sh`/`regen-active-rules.sh` invocations |
...
- Skill phase-file **procedures** (the steps each describes) — only the path/command tokens within them change (R13). The `spec` skill bundle.
...
- New assertion: after install, the old `flow/` dir does not exist and `.agents/` lists only `skills/`.
