...
async function copyRecursiveAsync(src: string, dest: string, force: boolean) {
  ...
  if (isDirectory) {
    ...
    if (fs.existsSync(dest) && !force) {
      ...
      if (response && (response as any).overwrite) {
...
function generateDirectoryMap(dir: string, indent: string = ''): string {
  let map = '';
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const stat = fs.statSync(filePath) as any;
...
program
  .name('agentkit')
  .description('A CLI tool to install AgentKit configurations into repositories.')
  .version('1.0.0');
...
program
  ...
  .action(async (kitName: string, options: { force?: boolean; target: string }) => {
    if (!kitName) {
